<x-app-layout>
                    ...
                    <tr class="block md:table-row hover:bg-gray-50 transition duration-150 p-4 border-b border-gray-200 md:border-b-0">
                        ...
                        <td class="block md:table-cell px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex gap-2 mt-2 md:mt-0"> {{-- Ajuste de margen en móvil --}}
                                ...
                                {{-- Restaurar --}}
                                <form action="{{ route('sectores.restaurar', $sector->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form action="{{ route('sectores.borrarDefinitivo', $sector->id) }}" method="POST" x-data>
                                    @csrf
                                    @method('DELETE')
    ...
    @foreach($items as $sector)
        <x-modal name="confirm-delete-{{ $sector->id }}" :show="false" maxWidth="md">
            <form action="{{ route('sectores.borrarDefinitivo', $sector->id) }}" method="POST" class="p-6">
                @csrf
                @method('DELETE')
