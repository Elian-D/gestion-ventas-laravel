<x-app-layout>
    <div class="max-w-7xl mx-auto">
        <div class="bg-white shadow-xl rounded-lg p-6">
            ...
            <h2 class="text-xl font-semibold text-gray-800 mb-6 border-b pb-3">Tipos de Negocios</h2>

            {{-- Toolbar --}}
            <div class="flex flex-col md:flex-row justify-between gap-4 mb-6">
                <form method="GET" class="w-full md:w-2/3 flex gap-2">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Buscar tipo de negocio..."
                ...
                <div class="flex gap-2">
                    <a href="{{ route('clients.negocios.eliminados') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md text-sm text-gray-700 bg-white hover:bg-gray-100">
                        <x-heroicon-s-trash class="w-5 h-5 mr-2" /> Papelera
                    </a>
                    <x-primary-button class="bg-green-600 hover:bg-green-700" x-data x-on:click="$dispatch('open-modal', 'crear-tipoNegocio')">
            ...
            <x-data-table
                ...
                @forelse($tipoNegocio as $negocio)
                    <tr class="block md:table-row hover:bg-gray-50 transition duration-150 p-4 border-b border-gray-200 md:border-b-0">
                        ...
                        <td class="block md:table-cell px-6 py-4 whitespace-nowrap text-sm font-medium w-full md:w-3/12">
                            <div class="flex items-center gap-3 mt-2 md:mt-0">
                                @php 
                                    $isLastActives = $negocio->activo && $tipoNegocio->where('activo', true)->count() <= 2; 
                                @endphp

                                <form action="{{ route('clients.negocios.toggle', $negocio) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <button type="submit" 
                                        {{ $isLastActives ? 'disabled' : '' }}
                                        class="text-xs px-2 py-1 rounded border {{ $isLastActives ? 'bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed' : ($negocio->activo ? 'bg-yellow-50 text-yellow-700 border-yellow-200 hover:bg-yellow-100' : 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100') }}">
                                        {{ $isLastActives ? 'Mínimo Activos' : ($negocio->activo ? 'Desactivar' : 'Activar') }}
                                    </button>
                                </form>

                                <button @click="$dispatch('open-modal', 'edit-tipoNegocio-{{ $negocio->id }}')" class="text-indigo-600 hover:text-indigo-900 p-1 rounded hover:bg-indigo-50">
                                    <x-heroicon-s-pencil class="w-5 h-5" />
                                </button>

                                <button 
                                    @if(!$isLastActives) @click="$dispatch('open-modal', 'confirm-deletion-{{ $negocio->id }}')" @endif
    ...
    @include('clients.negocios.modals') 
</x-app-layout>