                ...
                @forelse($items as $municipio)
                    ...
                    <tr class="block md:table-row hover:bg-gray-50 transition duration-150 p-4 border-b border-gray-200 md:border-b-0">
                        ...
                        <td class="block md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:text-gray-600">
                            ...
                            {{ $municipio->deleted_at->format('d/m/Y') }}
                        </td>
        ...
        <x-modal name="confirm-delete-{{ $municipio->id }}" :show="false" maxWidth="md">
            <form action="{{ route('geography.municipios.borrarDefinitivo', $municipio->id) }}" method="POST" class="p-6">
                ...
                <h2 class="text-lg font-medium text-gray-900">
                    ¿Eliminar definitivamente el municipio?
                </h2>

                <p class="mt-1 text-sm text-gray-600">
                    Esta acción no se puede deshacer. Municipio: 
                    <span class="font-bold text-red-600">{{ $municipio->nombre }}</span>.
                </p>
