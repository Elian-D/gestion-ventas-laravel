        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
                ...
                <h2 class="text-xl font-medium text-gray-700 mb-6 border-b pb-3">
                    Editar Municipio: <span class="text-indigo-600 font-semibold">{{ $municipio->nombre }}</span>
                </h2>

                <form action="{{ route('geography.municipios.update', $municipio) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <select name="provincia_id" id="provincia_id"
                            ...
                            @foreach($provincias as $provincia)
                                <option value="{{ $provincia->id }}" {{ old('provincia_id', $municipio->provincia_id) == $provincia->id ? 'selected' : '' }}>
                                    {{ $provincia->nombre }}
                                </option>
                            @endforeach
                        </select>
