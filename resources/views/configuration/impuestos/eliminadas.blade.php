<x-config-layout>
                    ...
                    <tr class="block md:table-row hover:bg-gray-50 transition duration-150 p-4 border-b border-gray-200 md:border-b-0">
                        ...
                        <td class="block md:table-cell px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex gap-2 mt-2 md:mt-0"> {{-- Ajuste de margen en móvil --}}
                                ...
                                {{-- Restaurar --}}
                                <form action="{{ route('configuration.impuestos.restaurar', $imp->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form action="{{ route('configuration.impuestos.borrarDefinitivo', $imp->id) }}" method="POST" x-data>
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" @click="$dispatch('open-modal', 'confirm-delete-{{ $imp->id }}')"
    ...
    @foreach($items as $imp)
        <x-modal name="confirm-delete-{{ $imp->id }}" :show="false" maxWidth="md">
            <form action="{{ route('configuration.impuestos.borrarDefinitivo', $imp->id) }}" method="POST" class="p-6">
                @csrf
                @method('DELETE')

                <h2 class="text-lg font-medium text-gray-900">
                    ¿Eliminar definitivamente el documento?
                </h2>

                <p class="mt-1 text-sm text-gray-600">
                    Esta acción no se puede deshacer. Tipo de documento: 
                    <span class="font-bold text-red-600">{{ $imp->nombre }}</span>.
                </p>
