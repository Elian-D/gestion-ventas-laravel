<x-config-layout>
                ...
                @forelse($items as $documento)
                    ...
                    <tr class="block md:table-row hover:bg-gray-50 transition duration-150 p-4 border-b border-gray-200 md:border-b-0">
                        ...
                        <td class="block md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:text-gray-600">
                            ...
                            {{ $documento->deleted_at->format('d/m/Y') }}
                        </td>
                        ...
                        <td class="block md:table-cell px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex gap-2 mt-2 md:mt-0"> {{-- Ajuste de margen en móvil --}}
                                ...
                                {{-- Restaurar --}}
                                <form action="{{ route('configuration.documentos.restaurar', $documento->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form action="{{ route('configuration.documentos.borrarDefinitivo', $documento->id) }}" method="POST" x-data>
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" @click="$dispatch('open-modal', 'confirm-delete-{{ $documento->id }}')"
    ...
    @foreach($items as $documento)
        <x-modal name="confirm-delete-{{ $documento->id }}" :show="false" maxWidth="md">
            <form action="{{ route('configuration.documentos.borrarDefinitivo', $documento->id) }}" method="POST" class="p-6">
                @csrf
                @method('DELETE')
